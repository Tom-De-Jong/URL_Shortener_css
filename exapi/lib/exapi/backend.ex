defmodule Exapi.Backend do
  ...
  def encode(url) do
    ...
    url = if not String.starts_with?(url, "http://") and not String.starts_with?(url, "https://"), do: "https://" <> url, else: url

    case Exapi.DB.read_encoded(url) do
      #Assigns encoded to the encoded, hashed string of the url, then is saved
      nil -> encoded = url |> :erlang.crc32() |> Base36.encode() |> String.downcase()
